...
### Requirement: 用户登录
系统 SHALL 提供 `POST /api/auth/login` 端点，验证凭据后返回 access_token、refresh_token 和用户信息。

#### Scenario: 登录成功
- **GIVEN** 用户 "zhangsan" 已注册，密码为 "pass1234"
- **WHEN** 客户端发送 `POST /api/auth/login`，body 为 `{"username": "zhangsan", "password": "pass1234"}`
