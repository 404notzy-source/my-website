...
### Requirement: 验证码生成
系统 SHALL 提供 `GET /api/auth/captcha` 端点，返回 4 位字母数字图片验证码。

#### Scenario: 获取验证码
- **GIVEN** 客户端请求验证码
- **WHEN** 发送 `GET /api/auth/captcha`
- **THEN** 系统返回 `{"captcha_id": "<hex>", "image": "data:image/png;base64,<base64>"}`，图片包含 4 位旋转字符 + 干扰线 + 噪点，不区分大小写
