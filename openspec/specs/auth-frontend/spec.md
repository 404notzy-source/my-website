...
#### Scenario: 登录成功后状态更新
- **GIVEN** AuthContext 初始化，isAuthenticated 为 false，验证码有效
- **WHEN** 调用 `login(username, password, captchaId, captchaAnswer)` 成功后
- **THEN** isAuthenticated 变为 true，user 对象包含用户信息，accessToken 存储于内存

#### Scenario: 注册成功后直接登录
- **GIVEN** 注册表单填写完整且验证码有效
- **WHEN** 调用 `register(username, email, password, captchaId, captchaAnswer)` 成功后
- **THEN** 后端返回 TokenResponse，前端直接设置登录态（无需再次调用 login）
...
- **GIVEN** 用户未登录
- **WHEN** 访问 `/login`
- **THEN** 系统渲染登录表单（用户名 + 密码 + 图片验证码 + 提交按钮 + "去注册"链接），验证码可点击刷新

#### Scenario: 注册页表单
- **GIVEN** 用户未登录
- **WHEN** 访问 `/register`
- **THEN** 系统渲染注册表单（用户名 + 邮箱 + 密码 + 确认密码 + 图片验证码 + 提交按钮 + "去登录"链接），验证码可点击刷新，校验密码一致性
